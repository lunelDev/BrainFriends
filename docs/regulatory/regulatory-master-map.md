...
2. `regulatory-master-map.md` — 전체 지도
3. `class-2-samd-readiness-matrix.md` — 2등급 SaMD 기준으로 무엇이 됐고 무엇이 부족한지
4. `article-24-dossier-index.md` — 제24조 첨부서류 8종별 내부 산출물/코드 증적/부족 항목
5. `source-digital-medical-product-notice-2026-requirements.md` — 제24~26조 허가 제출자료와 2등급 갈음 전략
6. `source-classification-grade-requirements.md` — 왜 SaMD/2등급 가능성인지
7. `source-gmp-requirements.md` — 품질·릴리스·형상관리로 무엇을 남겨야 하는지
8. `source-poststroke-dysarthria-dtx-requirements.md` — 마비말장애 DTx로 갈 때 어떤 임상지표가 필요한지
9. `source-ai-clinical-stroke-requirements.md` — AI 성능평가용 후향적 데이터셋을 어떻게 잠글지
10. `test-and-certification-development-guide.md` — 개발팀이 당장 고칠 항목
11. `claim-lock.md` — 회사/외부에 말해도 되는 문구와 금지 문구
