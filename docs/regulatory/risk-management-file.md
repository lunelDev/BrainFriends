...
| --- | --- | --- | --- | --- | --- | --- | --- |
| RM-001 | STT 전사 오류 | 발음장애·소음·마이크 품질 저하 → STT 오인식 → 수행 점수 또는 치료사 판단에 영향 | 잘못된 경과 해석, 부적절한 훈련 방향 | S3/P3 | STT 결과를 보조 전사로 제한, 치료사 검토 문구, useCase별 STT 정책 (training=WASM Whisper-tiny / evaluation=server Whisper), 도메인 prompt, 오류 가능성 고지, 실패·빈 전사·서버 차단 시 `reviewRequired=true` | 조건부 수용 | `SR-STT-009`, `sttPolicy.ts`, `sttPrompt.ts`, `sttReview.ts`, `wasmSttAdapter.ts` (transformers.js@4.2.0:Xenova/whisper-tiny:v0.1), `TC-RISK-001`, `TC-STT-WASM-001`, AI 평가 문서 |
| RM-002 | 서버 STT 데이터 전송 경계 오해 | 제품 전체를 온디바이스로 설명 → 실제 STT 전송 발생 → 개인정보·보안 심사 불일치 | 민감정보 처리 고지 미흡, 심사 보완 | S2/P2 | 클레임 잠금표에서 전체 원본 미전송 금지, 데이터 처리 경계 문서화, STT useCase 구분, **training useCase 는 WASM 온디바이스로 wiring 완료** (rawAudioLeavesDevice=false 결정성 검증) | 수용 가능 (training useCase 한정) / 조건부 수용 (evaluation useCase) | `claim-lock.md` v0.1.7 §3, `cloud-and-data-transfer.md`, `wasmSttAdapter.ts`, `TC-STT-WASM-001` |
