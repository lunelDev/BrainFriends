...
관련: `werRunner.ts` (`SR-AI-EVAL-RUNNER`), `sttBenchmark.ts` (`SR-AI-RTF-RUNNER`), `wasm-stt-model-evaluation-plan.md`
...
본 문서는 데이터 수집 가이드이다. 평가 결과는 `npm run ai-eval:wer` + `npm run ai-eval:rtf` 로 자동 산출되어 `docs/remediation/03-ai-evaluation/runs/<timestamp>/report.{json,md}` 에 저장된다.
...
5. STT 결과 (transcript) 수집:
   - WASM Whisper-tiny 모드: 브라우저에서 `transcribeWithWasmStt` 호출 결과
   - 서버 Whisper 모드: `/api/proxy/stt` 응답
...
npm run ai-eval:wer -- \
  ...
  --model wasm:Xenova/whisper-tiny \
  --timestamp 2026-05-15T09:00:00.000Z
```

출력:
- `docs/remediation/03-ai-evaluation/runs/2026-05-15T09-00-00-000Z/report.json`
- `docs/remediation/03-ai-evaluation/runs/2026-05-15T09-00-00-000Z/report.md`
...
출력:
- `docs/remediation/03-ai-evaluation/runs/rtf-2026-05-15T09-00-00-000Z/report.{json,md}`
...
| byAgeGroup 80s passRateAt15 | ≥ 0.65 | wasm-stt-model-evaluation-plan.md §3.2 |
| meanRtf | < 1.0 | wasm-stt-model-evaluation-plan.md §3.1 |
| p95LatencyMs | ≤ 41.5 ms | 제품기획서 정량 클레임 |
| passRateP95Target | ≥ 0.85 | wasm-stt-model-evaluation-plan.md §3.1 |
...
- 보고서 (`docs/remediation/03-ai-evaluation/runs/`): 영구 보관 (claim-lock §3 사이버보안·형상관리·변경관리 행 증거)
