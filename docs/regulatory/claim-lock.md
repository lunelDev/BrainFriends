...
KTL 1차 미팅 이후 사업 실행 전략은 `1등급 정보 제공/훈련 보조 우선 신고 + 2등급 SaMD 장기 준비` 투트랙을 우선 검토한다. 따라서 같은 기능이라도 외부 설명 문구는 트랙에 따라 분리한다.

### 3.1 분기 원칙

| 구분 | 1등급 우선 신고용 | 2등급 SaMD 준비용 |
| --- | --- | --- |
| 제품 성격 | 정보 제공, 자기관리, 훈련 보조, 발화 보조 | 독립형 디지털의료기기소프트웨어, 재활 보조 SaMD |
| 사용목적 표현 | 수행 기록, 자가점검, 훈련 활동, 의사표현 보조 | 언어재활 훈련 보조, 경과 모니터링, 치료사 검토 보조 |
| 결과 표현 | 참고 지표, 기록, 요약, 전문가 확인 자료 | 임상 성능평가 대상 보조 지표, 치료사 검토용 결과 |
| AI 표현 | 자동 판단 아님, 참고값 산출, 전문가 검토 보조 | AI 기반 보조 지표 산출, 성능평가 대상 파이프라인 |
| 금지 강도 | 치료·재활·개선·효과·진단·처방 표현 금지 | 진단·처방·치료 결정 대체 금지, 효과 입증 표현은 임상 전까지 금지 |
| 데이터 표현 | 사용자가 입력한 수행 기록과 참고값을 저장·조회 | V&V, AI 성능평가, 위험관리, GMP 증빙과 연결 |

### 3.2 1등급 우선 신고용 허용 표현

아래 표현은 1등급 정보 제공/훈련 보조 트랙에서 우선 사용한다.

| 영역 | 허용 표현 | 사용 예 |
| --- | --- | --- |
| 제품 소개 | 발화 훈련 보조 서비스 | 브레인프렌즈는 사용자의 발화 활동을 기록하고 전문가 확인을 돕는 훈련 보조 서비스입니다. |
| 자가점검 | 자가점검, 상태 확인, 수행 기록 | 자가점검 결과를 기준으로 강점과 추가 확인이 필요한 항목을 보여줍니다. |
| 언어 활동 | 언어 훈련, 발화 활동, 반복 연습 | 사용자는 제시 문장을 듣고 따라 말하는 발화 활동을 수행합니다. |
| 노래훈련 | 노래 활동, 음악·리듬 기반 발화 활동 | 노래 활동에서 발화 참여율과 타이밍 참고 지표를 기록합니다. |
| AAC | 의사표현 보조, 심볼 기반 문장 생성 | 말로 표현하기 어려울 때 심볼을 선택해 의사를 전달합니다. |
| 결과 | 참고 지표, 수행 요약, 기록 리포트 | 결과는 자동 판정이 아니라 전문가 확인을 위한 참고 지표입니다. |
| AI/STT | 자동 전사 참고값, 음성 입력 보조 분석 | 자동 전사 결과는 참고값이며 정확한 판단은 전문가가 확인합니다. |
| 안면/시선 | 화면 응시, 얼굴 반응, 구강 움직임 참고값 | 카메라 기반 얼굴 반응 참고값을 함께 기록합니다. |

### 3.3 1등급 우선 신고용 금지 표현

아래 표현은 1등급 우선 신고용 화면, 소개자료, 리포트, 사업자료에서 사용하지 않는다.

| 금지 표현 | 치환 표현 |
| --- | --- |
| 치료 | 훈련, 활동, 전문가 확인 |
| 재활 | 발화 훈련, 언어 활동, 수행 기록 |
| 언어 회복 | 발화 활동 기록, 반복 연습 |
| 개선 효과 | 전후 참고값 변화, 수행 기록 변화 |
| 진단 | 자가점검, 상태 확인, 전문가 확인 필요 |
| 처방 | 권장 활동, 다음 활동 |
| 치료사 판단을 대체 | 전문가 검토 보조 |
| 자동 평가 | 참고 지표 산출 |
| AI 진단 | AI 기반 참고값 산출 |
| 임상 효과 입증 | 임상 근거 검증 예정 |

### 3.4 2등급 SaMD 준비용 허용 표현

아래 표현은 2등급 가능성 기준의 내부 개발 문서, 허가 준비 문서, 시험기관 대응 문서에서 사용할 수 있다. 단, 대외 광고·홈페이지 문구에는 임상 근거 확보 전까지 제한적으로 사용한다.

| 영역 | 허용 표현 | 제한 조건 |
| --- | --- | --- |
| 제품 유형 | 독립형 디지털의료기기소프트웨어(SaMD)로 개발 중 | 허가 완료 표현 금지 |
| 등급 | 2등급 가능성을 기준으로 준비 | 최종 등급 확정 표현 금지 |
| 사용목적 | 언어재활 훈련과 경과 모니터링을 보조 | 치료 결정 대체 금지 |
| 임상 | 확증적 임상시험을 통해 유효성 검증 예정 | 효과 입증 표현 금지 |
| AI | 치료사 검토용 보조 지표를 산출하는 AI 파이프라인 | 자동 진단 금지 |
| 성능평가 | WER/CER/RTF 등 분석성능을 검증 예정 | 실측 전 달성 표현 금지 |
| 위험관리 | ISO 14971 기반 위험통제와 잔여위험 관리 | 문서/테스트 증빙 필요 |
| V&V | IEC 62304 요구사항-설계-구현-검증 추적성 관리 | traceability와 test 결과 필요 |
| 사이버보안 | 식약처 사이버보안 요구사항 기반 보안 통제 | 운영 서버 증빙은 별도 필요 |

### 3.5 분기 적용 대상

| 대상 | 기본 적용 |
| --- | --- |
| 환자용 앱 화면 | 1등급 우선 신고용 표현 |
| 보호자 화면 | 1등급 우선 신고용 표현 |
| 홈페이지/브로셔/IR 자료 | 1등급 우선 신고용 표현. 2등급 문구는 “개발 중/준비 중”으로 제한 |
| 치료사/관리자 화면 | 2등급 준비용 표현 가능. 단 자동 진단·처방 표현은 금지 |
| 결과 리포트 | “전문가/치료사 검토용 참고 지표”를 기본 문구로 고정 |
| 식약처/시험기관/RA 내부 문서 | 2등급 SaMD 준비용 표현 사용 가능 |
| 임상시험/IRB 문서 | 승인받은 사용목적과 임상 프로토콜 문구 우선 |
...
| 노래훈련 | 노래훈련은 MIT 및 음악 기반 언어재활 선행연구를 참고한 음악·리듬 기반 발화 보조 훈련 콘텐츠이다. 노래방 UX는 유지하되 총점은 STT 전사 정확도 중심이 아니라 발화 참여율, 가사 타이밍 맞춤률, 발성 안정도, 반응시간, 안면-음성 동시성 중심으로 산출한다. STT 기반 자음·모음·전사 결과는 치료사 검토용 참고값으로만 제공한다. | `src/app/(training)/programs/sing-training/page.tsx`, `src/app/(result)/result-page/sing-training/page.tsx`, `src/features/sing-training/data/songs.ts`, sing result ZIP export |
...
| 변경 후 재시험·영향평가 export | release manifest delta, anomaly list, retest result, impact assessment 를 `latest-change-dossier.json/md/csv` 로 산출해 제26조 SW 변경 후 증빙 묶음으로 관리한다. | `SR-CHANGE-016`, `lib/server/releaseChangeDossier.ts`, `scripts/generate-release-change-dossier.ts`, `docs/security/manifest/latest-change-dossier.*`, `TC-CHANGE-DOSSIER-001` |
...
| WASM-STT 격리 | WASM-STT는 제품 훈련 경로에서 사용하지 않고 `/dev/wasm-stt-test` 개발 검증 화면과 adapter 코드에만 격리한다. `NEXT_PUBLIC_STT_WASM_EXPERIMENT` 같은 환경변수로 일반 훈련 화면이 WASM-STT를 선택하지 못한다. | `lib/speech/sttPolicy.ts`, `lib/speech/sttRuntime.ts`, `lib/speech/SpeechAnalyzer.ts`, `app/dev/wasm-stt-test/page.tsx`, `TC-STT-001/002/003`, `TC-STT-WASM-001` |
| 적응형 난이도 (IRT 결정성) | 2PL Item Response Theory 모델 (a, b parameter + θ EAP 추정) + Maximum Fisher Information 문항 선택을 결정성 함수로 산출한다. 임상 calibration (item bank a/b 추정) 후 §5 "Bayesian Adaptive Testing 적용" 클레임 §4 로 완전 승격 가능. | `SR-IRT-018`, `lib/adaptive/irt.ts` (irt:2pl-mfi:v0.1), `TC-IRT-001` |
...
| 노래방 훈련 운율·음정 재활 | 음악·리듬 기반 노래 훈련. 노래방 UX 안에서 발화 참여율, 가사 타이밍 맞춤률, 발성 안정도, 반응시간, 안면-음성 동시성을 산출하고 STT 전사는 참고값으로 제공 | 음정 치료·pitch 개선·치료효과 확정 표현 제거 |
...
- AI STT 성능평가 신규 실측값 입력 (`npm run ai-eval:wer`) → §5 WER 행 검토 + risk-management RM-001 P 값 재산정 트리거
- STT 성능 벤치마크 (RTF/P95) 신규 실측값 입력 (`npm run ai-eval:rtf`) → §5 STT 행 + 제품기획서 P95 41.5ms 클레임 검증 트리거
...
- IRT item bank 신규 calibration (실제 환자 응답 기반 a/b 추정) → claim-lock §5 적응형 난이도 행 → §4 완전 승격 검토 트리거
- 사용성평가 summative 15명 실시 결과 수집 → risk-management v1.x 잔여위험 재산정 + claim-lock §4 IEC 62366 행 갱신
- 임상 협력기관 30건 음성·전사 입력 → npm run ai-eval:wer 실행 → claim-lock §5 WER 행 → §4 승격 검토
- STT 엔진·정책 변경 또는 내부 실측 결과 수령 → claim-lock §5 STT/WER 행 검토
- GMP/QMS 외주사 견적 수령 → gmp-qms-decision-matrix §5 가중치 재산정
- 노래훈련 pitch tracker, 리듬 동기화 지표, MIT 유사 프로토콜, 내부 전/후 비교 결과 또는 임상시험 결과 추가 → 노래훈련 클레임 §4/§5/§6 재검토
...
- 2026-05-13: KTL 1차 미팅 후속으로 1등급 우선 신고용 / 2등급 SaMD 준비용 문구 분기표 추가. 1등급 허용어·금지어·치환표와 적용 대상을 정의하고, 기존 2등급 가능성 기준 클레임은 내부 허가 준비 문구로 유지.
- 2026-05-06: 노래훈련 클레임 잠금 추가. 제품기획서의 "노래방 훈련 운율·음정 재활" 표현을 "MIT 및 음악 기반 언어재활 선행연구를 참고한 음악·리듬 기반 발화 보조 훈련"으로 치환하도록 고정. 사용 가능 문구는 발화 참여율, 가사 타이밍 맞춤률, 발성 안정도, 반응시간, 안면-음성 동시성, STT 자동 전사 참고값을 치료사 검토용 보조 지표로 제공하는 범위로 제한. 금지 클레임에 언어 회복 효과 입증, 정식 MIT 자동 치료, 음정/pitch 개선 효과 입증을 추가.
- 2026-05-08: 노래훈련 점수 판정 기준 정정. 목표 가사를 STT prompt bias 로 넣지 않고, 총점은 `sing-score-v3-performance-weighted` 기준으로 발화 참여율, 가사 타이밍 맞춤률, 발성 안정도, 반응시간, 안면-음성 동시성 중심으로 산정한다. STT 기반 자음·모음·전사 일치도는 자동 전사 참고값으로만 남기며, 녹음 실패/수행 샘플 부족/관리자 skip 은 서버 저장·랭킹 반영에서 제외하고 결과 ZIP의 `scoring-evidence.json`에 판정 근거를 남긴다.
- 2026-05-11: WASM-STT 제품 경로 격리 완료. 일반 훈련 화면(step-2/4/5, 노래훈련, 문장 게임)에서 WASM-STT 로딩 UI와 lifecycle callback 제거. `SpeechAnalyzer`는 서버 보안 프록시 STT만 사용하며, WASM-STT adapter는 `/dev/wasm-stt-test` 개발 검증 화면에서만 직접 호출한다. V&V는 브라우저 WASM 사용 가능 여부와 관계없이 제품 STT runtime 이 `server_whisper`로 고정됨을 검증한다.
- 2026-05-11: Release Change Dossier 추가. `buildReleaseChangeDossier()`가 release manifest 이전/다음 hash, anomaly list, retest result, impact assessment, release gate를 결정성으로 산출한다. `npm run security:change-dossier`로 `docs/security/manifest/latest-change-dossier.json/md/csv`를 생성하며, V&V `TC-CHANGE-DOSSIER-001`에 포함했다.
