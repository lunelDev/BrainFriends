...
현재 SQL 초안:
- [brainfriends_dev.sql](./database/brainfriends_dev.sql)
...
현재 적용 초안:
- [singResultsDb.ts](../src/lib/server/singResultsDb.ts)
  - `patient_pii`, `patient_pseudonym_map`, `clinical_sessions`, `sing_results` 순서로 저장
- [clinicalResultsDb.ts](../src/lib/server/clinicalResultsDb.ts)
  - `patient_pii`, `patient_pseudonym_map`, `clinical_sessions`, `language_training_results` 순서로 저장
- [auditLog.ts](../src/lib/server/auditLog.ts)
  - `patient_pseudonym_id` 기준 NDJSON 감사로그 기록
