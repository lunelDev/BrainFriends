...
권장 운영 구조는 PostgreSQL `audit_logs` 테이블 또는 변경 불가능한 object storage 보관입니다.
...
권장 운영 구조:
- 1차 저장: PostgreSQL `audit_logs` 테이블
...
- `docs/database/brainfriends_dev.sql`
  - 향후 DB 운영을 위한 `audit_logs` 테이블 초안 추가
