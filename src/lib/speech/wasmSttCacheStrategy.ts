...
//
// WASM-STT 모델/런타임은 public 정적 자산으로 고정한다.
// Service Worker 레벨에서 /models/wasm-stt/*, /vendor/onnxruntime/* 를 cache-first
// 로 잡아두면 (1) 오프라인 동작, (2) 첫 로딩 진행률 표시 통합이 가능하다.
//
...
export function decideCacheStrategy(input: CacheStrategyInput): CacheDecision {
  ...
  // Whisper 모델/ONNX 런타임 자산 — cache-first (한번 받으면 영구)
