...
export default function LingoResultModalShell({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-start justify-center overflow-y-auto bg-slate-900/90 p-4 backdrop-blur-xl sm:items-center sm:p-6">
      <div
        className={`relative my-auto w-full ${maxWidthClass} max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-[56px] border-[6px] border-violet-100 bg-white p-8 shadow-2xl sm:max-h-[calc(100dvh-3rem)] sm:p-10`}
      >
        <div className={`text-center ${hideIcon ? "mb-8" : "mb-10"}`}>
          {!hideIcon ? (
            <div className={`mb-6 inline-block rounded-[40px] p-6 text-white shadow-2xl ${iconToneClass}`}>
        ...
        <div className="space-y-3">
          <button
            onClick={onPrimary}
            className={`h-16 w-full rounded-[24px] text-lg font-black text-white shadow-xl transition-transform active:scale-95 ${primaryButtonClass}`}
          >
            {primaryLabel}
          </button>
          {secondaryLabel && onSecondary ? (
            <button
              onClick={onSecondary}
              className={`flex h-16 w-full items-center justify-center rounded-[24px] text-sm font-black transition-all ${secondaryButtonClass}`}
            >
              {secondaryLabel}
            </button>
          ) : null}
