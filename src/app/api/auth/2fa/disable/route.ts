// src/app/api/auth/2fa/disable/route.ts
//
// POST /api/auth/2fa/disable
//   body: { code: string }
//   본인이 현재 활성화된 TOTP 를 해제. 해제 시 코드 1회 검증 필수.
//
// 정책:
//   - 처방자/관리자는 disable 후 다음 로그인부터 TOTP 미요구
//     (재등록 전까지 단일요인 로그인 — 위험 신호이므로 가능하면
//     관리자 검토 필요)
//   - 코드 검증 통과 못 하면 disable 거부.

import { cookies } from "next/headers";
import { NextResponse } from "next/server";
import {
  AUTH_COOKIE_NAME,
  getAuthenticatedSessionContext,
} from "@/lib/server/accountAuth";
import { verifyTotp } from "@/lib/server/totp";
import {
  getUserTotp,
  recordTotpFailure,
  isLockedNow,
  disableTotp,
} from "@/lib/server/userTotpDb";
import { safeAppendAccess } from "@/lib/server/auditLog";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

export async function POST(req: Request) {
  const cookieStore = await cookies();
  const token = cookieStore.get(AUTH_COOKIE_NAME)?.value;
  if (!token) {
    return NextResponse.json({ ok: false, error: "unauthorized" }, { status: 401 });
  }
  const ctx = await getAuthenticatedSessionContext(token);
  if (!ctx) {
    return NextResponse.json({ ok: false, error: "unauthorized" }, { status: 401 });
  }

  let body: Record<string, unknown>;
  try {
    body = (await req.json()) as Record<string, unknown>;
  } catch {
    return NextResponse.json({ ok: false, error: "invalid_json" }, { status: 400 });
  }
  const code = typeof body.code === "string" ? body.code.trim() : "";
  if (!code) {
    return NextResponse.json({ ok: false, error: "missing_code" }, { status: 400 });
  }

  try {
    const totp = await getUserTotp(ctx.userId);
    if (!totp || !totp.enabledAt) {
      return NextResponse.json(
        { ok: false, error: "not_enabled" },
        { status: 404 },
      );
    }
    if (isLockedNow(totp)) {
      return NextResponse.json(
        { ok: false, error: "locked_too_many_failures" },
        { status: 429 },
      );
    }

    const verified = verifyTotp({
      secretBase32: totp.secretBase32,
      code,
      digits: totp.digits,
      periodSec: totp.periodSec,
      algorithm: totp.algorithm,
    });
    if (verified === null) {
      await recordTotpFailure(ctx.userId);
      await safeAppendAccess({
        request: req,
        action: "delete",
        status: "failed",
        operatorUserId: ctx.userId,
        operatorUserRole: ctx.userRole,
        subjectUserId: ctx.userId,
        resourceType: "user_2fa_totp",
        resourceId: ctx.userId,
        httpStatus: 401,
        failureReason: "invalid_totp_code",
      });
      return NextResponse.json(
        { ok: false, error: "invalid_code" },
        { status: 401 },
      );
    }

    await disableTotp(ctx.userId);
    await safeAppendAccess({
      request: req,
      action: "delete",
      operatorUserId: ctx.userId,
      operatorUserRole: ctx.userRole,
      subjectUserId: ctx.userId,
      resourceType: "user_2fa_totp",
      resourceId: ctx.userId,
      httpStatus: 200,
    });
    return NextResponse.json({ ok: true, disabled: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : "disable_failed";
    console.error("[2fa/disable][POST]", message);
    return NextResponse.json({ ok: false, error: message }, { status: 500 });
  }
}
