...
import { buildAiEvaluationEvidenceSummary } from "@/lib/server/aiEvaluationEvidenceDb";
...
export async function GET() {
  ...
  const summary = await buildAiEvaluationEvidenceSummary();
  return new NextResponse(JSON.stringify(summary, null, 2), {
    status: 200,
    headers: {
      "Content-Type": "application/json; charset=utf-8",
      "Content-Disposition": 'attachment; filename="brainfriends-ai-evaluation-report.json"',
    },
  });
}
